            ...
            """
{
  ...
  "type": "release"
    ...
    fun loadLaunchInfoLooping() {
        Main(appDataDir, Platform.MacOS).loadLaunchInfo("looping2")
    }

    @Test(expected = KnownErrorException.VersionJsonNotFile::class)
    fun loadVersionJsonNotFound() {
        Main(appDataDir, Platform.MacOS).loadVersionJson("notfound")
    }

    @Test(expected = KnownErrorException.InvalidVersionJsonData::class)
    fun loadVersionJsonInvalidJsonData() {
        Main(appDataDir, Platform.MacOS).loadVersionJson("invalidJsonData1")
    }

    @Test(expected = KnownErrorException.InvalidVersionJson::class)
    fun loadVersionJsonInvalidJson() {
        Main(appDataDir, Platform.MacOS).loadVersionJson("invalidJson1")
    }

    @Test
    fun loadVersionJson() {
        val json = Main(appDataDir, Platform.MacOS).loadVersionJson("inheritingJsonData1")
        assertEquals(json.id, "inheritingJsonData1")
    }

    @Test
    fun getLoadArtifacts() {
        val loadArtifacts = Main(appDataDir, Platform.MacOS)
            .getLoadArtifacts(
    ...
    @Test
    fun createClassPath() {
        val main = spyk(Main(appDataDir, Platform.MacOS))
        val launchInfo =
            LaunchInfo.Builder("jsonName").apply {
                addVersionJson(
                    VersionJson(
